...
def test_number_field_type(api_client, data_fixture):
    ...
    large_valid_int = "99999999999999999999999999999999999999999999999999"
    small_valid_int = "-99999999999999999999999999999999999999999999999999"
    response = api_client.post(
        reverse("api:database:rows:list", kwargs={"table_id": table.id}),
        {
            f"field_{positive_int_field_id}": large_valid_int,
            f"field_{negative_int_field_id}": small_valid_int,
    ...
    response_json = response.json()
    assert response.status_code == HTTP_200_OK
    assert response_json[f"field_{positive_int_field_id}"] == large_valid_int
    assert response_json[f"field_{negative_int_field_id}"] == small_valid_int
    ...
    model = table.get_model(attribute_names=True)
    row = model.objects.all().last()
    assert row.positiveintedit == Decimal(large_valid_int)
    assert row.negativeint == Decimal(small_valid_int)
    ...
    large_invalid_int = "999999999999999999999999999999999999999999999999999"
    response = api_client.post(
        reverse("api:database:rows:list", kwargs={"table_id": table.id}),
        {
            f"field_{positive_int_field_id}": large_invalid_int,
        },
        format="json",
        HTTP_AUTHORIZATION=f"JWT {token}",
    )
    ...
    small_invalid_int = "-9999999999999999999999999999999999999999999999999999"
    response = api_client.post(
        reverse("api:database:rows:list", kwargs={"table_id": table.id}),
        {
            f"field_{negative_int_field_id}": small_invalid_int,
        },
        format="json",
        HTTP_AUTHORIZATION=f"JWT {token}",
    )
